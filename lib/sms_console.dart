...
class _SmsConsolePageState extends State<SmsConsolePage> {
  ...
  Future<void> loadCosts() async {
    ...
    final res = await http.get(Uri.parse('$kApiBase/api/v1/sms/cost/breakdown'), headers: {'Authorization': 'Bearer $kApiKey'});
  ...
  Future<void> sendSms() async {
    ...
    try {
      ...
      final res = await http.post(Uri.parse('$kApiBase/api/v1/sms/send'), headers: {'Authorization': 'Bearer $kApiKey', 'Content-Type': 'application/json'}, body: jsonEncode({'to': phone, 'body': body}));
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Sent via $provider — €${cost.toStringAsFixed(4)}')));
          ...
          : Padding(
              ...
              child: Column(
                children: [
                  ...
                  Expanded(
                    child: FutureBuilder(
                      future: http.get(Uri.parse('$kApiBase/api/v1/sms/cost/breakdown'), headers: {'Authorization': 'Bearer $kApiKey'}),
                      builder: (context, snapshot) {
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            return ListTile(title: Text(rows[i]['provider']), subtitle: Text(rows[i]['recipient']), trailing: Text('€${rows[i]['totalCost']}'));
                          },
                        );
                      },
                    ),
